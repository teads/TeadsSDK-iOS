...
@protocol TeadsPrebidAdPlacement;
...
/// note:
/// This method is thread-safe and can be called from any thread (e.g. AdMob adapter background setUp)
...
SWIFT_CLASS("_TtC8TeadsSDK28TeadsAdPlacementInterstitial")
@interface TeadsAdPlacementInterstitial : NSObject <TeadsAdPlacementIdentifiable>
@property (nonatomic, readonly, copy) NSString * _Nonnull placementId;
@property (nonatomic, readonly) enum TeadsAdPlacementType placementType;
...
- (void)invalidate;
/// Load the interstitial ad. Call this before show().
- (void)loadAd;
/// Present the interstitial fullscreen
/// \param viewController The view controller to present from. Falls back to top VC if nil.
///
...
@interface TeadsAdPlacementInterstitial (SWIFT_EXTENSION(TeadsSDK)) <SFSafariViewControllerDelegate>
- (void)safariViewControllerDidFinish:(SFSafariViewController * _Nonnull)controller;
@end

@interface TeadsAdPlacementInterstitial (SWIFT_EXTENSION(TeadsSDK))
SWIFT_CLASS_PROPERTY(@property (nonatomic, class) BOOL testDisplay;)
+ (BOOL)testDisplay SWIFT_WARN_UNUSED_RESULT;
+ (void)setTestDisplay:(BOOL)value;
@end
...
/// Never create your own TeadsAdapterIntegrationType directly. Use one of the predefined standard values.
SWIFT_CLASS("_TtC8TeadsSDK27TeadsAdapterIntegrationType")
@interface TeadsAdapterIntegrationType : NSObject
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
///   </li>
///   <li>
///     <a href="https://support.teads.tv/support/solutions/articles/36000357700-inread-applovin-mediation">AppLovin</a>
///   </li>
///   <li>
...
/// Specify integration type and version
/// When using mediation adapters or plugin, provide a integration data to monitor them
/// important:
...
typedef SWIFT_ENUM(NSInteger, TeadsFullScreenEventName, open) {
  TeadsFullScreenEventNameWillPresent = 0,
  TeadsFullScreenEventNamePresented = 1,
  TeadsFullScreenEventNameWillDismiss = 2,
  TeadsFullScreenEventNameDismissed = 3,
};

/// Delegate for fullscreen placement lifecycle events.
/// Inherits from <code>TeadsAdPlacementEventsDelegate</code> so consumers only need
/// to conform to this single protocol to receive both ad and lifecycle events.
SWIFT_PROTOCOL("_TtP8TeadsSDK29TeadsFullScreenEventsDelegate_")
@protocol TeadsFullScreenEventsDelegate <TeadsAdPlacementEventsDelegate>
...
/// Specify a unique <code>tag</code> value for each UI element on InterfaceBuilder: make sure to match them using builder
/// \code
...
/// }
///
/// \endcode\param ad The ad that should be binded to the ad view.
///
/// \param tagBinder Binds the native asset ad views to this native ad using view tags.
///
...
@protocol TeadsPrebidAdPlacement <TeadsAdPlacement>
/// TeadsInReadAdPlacementDelegate to follow ad placement lifecycle
@property (nonatomic, strong) id <TeadsInReadAdPlacementDelegate> _Nullable delegate;
/// Load an InRead ad on this placement by passing adResponse value
/// listen for events by implementing <code>TeadsInReadAdPlacementDelegate</code>
/// requires:
...
/// \param adResponse Prebid winning bid ad response string representation (stringified JSON)
///
/// \param requestSettings settings <code>TeadsInReadAdRequestSettings</code> to tweak your needs
///
///
